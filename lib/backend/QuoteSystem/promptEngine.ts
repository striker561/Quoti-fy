import { MoodPromptInput } from "@/types/shared";
...
function buildQuotePrompt(input: MoodPromptInput): string {
  const intensity = describeIntensity(input.moodScale);
  return `
  Generate an emotionally moving quote that reflects a sense of ${
    input.mood
  } at ${intensity}.
...
function buildImagePrompt(input: MoodPromptInput): string {
  const intensity = describeIntensity(input.moodScale);
  return `
  Create a ${input.imageStyle.toLowerCase()} background image that visually expresses ${
    input.mood
  } with ${intensity}.
  Avoid including any text.
  Incorporate elements of ${input.timeOfDay.toLowerCase()} lighting and atmosphere, appropriate for ${
    input.location
  }.
