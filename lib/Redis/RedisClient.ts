...
export class RedisClient {
    ...
    async incrBy(key: string, count: number): Promise<number | null> {
        try {
            if (!this.redis_client) return null;
            const newValue = await this.redis_client.incrby(key, count);
            return newValue
        } catch (error) {
    ...
    async setExp(key: string, exp: number): Promise<number | null> {
        try {
            if (!this.redis_client) return null;
            const expiry = await this.redis_client.expire(key, exp);
            return expiry
        } catch (error) {
            console.error(`Redis INCRBY error: ${error}`);
            return null;
